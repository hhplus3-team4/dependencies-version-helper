...
public class GradleCleanerWithGroovy implements GradleCleaner {
    ...
    private String removeDependencyVersions(String fileContent, List<Dependency> selectedDependencies) {
        ...
        for (Dependency dependency : selectedDependencies) {
            ...
            String patternString = "(" + dependencyTypes + ")\\s+'\"?" + Pattern.quote(dependency.getGroupId()) +
                    ":" + Pattern.quote(dependency.getArtifactId()) + ":[^'\"\\s]+['\"]?";

            modifiedContent = modifiedContent.replaceAll(patternString, "$1 '" + dependency.getGroupId() + ":" +
